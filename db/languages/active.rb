...
[
  ...
  },
  {
    ...
    is_archived: false,
    source_file: "Main.cs",
    compile_cmd: "mkdir -p .dotnet-home/.nuget/packages out && printf '%%s\\n' '{ \"sdk\": { \"version\": \"8.0.420\", \"rollForward\": \"disable\" } }' > global.json && printf '%%s\\n' '<Project Sdk=\"Microsoft.NET.Sdk\">' '  <PropertyGroup>' '    <OutputType>Exe</OutputType>' '    <TargetFramework>net8.0</TargetFramework>' '    <ImplicitUsings>disable</ImplicitUsings>' '    <Nullable>disable</Nullable>' '    <UseAppHost>false</UseAppHost>' '  </PropertyGroup>' '</Project>' > Main.csproj && DOTNET_ROOT=/usr/local/dotnet-sdk DOTNET_CLI_HOME=$(pwd)/.dotnet-home DOTNET_MULTILEVEL_LOOKUP=0 DOTNET_NOLOGO=1 DOTNET_CLI_TELEMETRY_OPTOUT=1 DOTNET_GENERATE_ASPNET_CERTIFICATE=false DOTNET_ADD_GLOBAL_TOOLS_TO_PATH=false DOTNET_CLI_WORKLOAD_UPDATE_NOTIFY_DISABLE=1 DOTNET_SKIP_WORKLOAD_INTEGRITY_CHECK=1 NUGET_PACKAGES=$(pwd)/.dotnet-home/.nuget/packages dotnet build Main.csproj --framework net8.0 -nologo -o out %s >/dev/null",
    run_cmd: "DOTNET_ROOT=/usr/local/dotnet-sdk DOTNET_CLI_HOME=$(pwd)/.dotnet-home DOTNET_MULTILEVEL_LOOKUP=0 DOTNET_NOLOGO=1 DOTNET_CLI_TELEMETRY_OPTOUT=1 DOTNET_GENERATE_ASPNET_CERTIFICATE=false DOTNET_ADD_GLOBAL_TOOLS_TO_PATH=false DOTNET_CLI_WORKLOAD_UPDATE_NOTIFY_DISABLE=1 DOTNET_SKIP_WORKLOAD_INTEGRITY_CHECK=1 NUGET_PACKAGES=$(pwd)/.dotnet-home/.nuget/packages dotnet ./out/Main.dll"
  },
  {
    id: 3008,
    name: "C# (.NET 10.0.202)",
